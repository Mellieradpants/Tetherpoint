...
function ExternalIcon() {
  return (
    <svg
      ...
      viewBox="0 0 11 11"
      fill="none"
      style={{ marginLeft: 3, opacity: 0.6, display: "inline" }}
    >
      <path
      ...
      <span style={{ fontSize: 13, color: "#3d3530", lineHeight: 1.55, flex: 1 }}>
        {url ? (
          <a
            ...
            style={{ color: "#5c4a3a", textDecoration: "underline", textDecorationStyle: "dotted" }}
          >
            {value}
            <ExternalIcon />
          </a>
    ...
    <>
      <div
        style={{
          ...
          marginBottom: 8,
          boxShadow: "0 2px 14px rgba(193,127,58,0.08)",
        }}
      >
        ...
        {translationState === "error" && (
          ...
          >
            Translation unavailable. Showing original meaning.
          </p>
        )}
        {answerLanguage !== "en" && translationState !== "error" && (
          <p
            style={{
              marginTop: 8,
              fontSize: 11,
          ...
          >
            Language: {languageLabel(answerLanguage)}
          </p>
        )}
      </div>
      ...
      <CollapsiblePanel title="Origin" accent="#4a6fa5" defaultOpen={true}>
        {active.origin.title && <MetaRow label="Title" value={active.origin.title} />}
        {active.origin.sourceName && <MetaRow label="Name" value={active.origin.sourceName} />}
        <MetaRow label="Source" value={active.origin.source} url={active.origin.url} />
        <MetaRow label="Anchor" value={active.origin.statute} />
        {active.origin.documentId && <MetaRow label="Doc ID" value={active.origin.documentId} />}
        {active.origin.sourcePath && <MetaRow label="Path" value={active.origin.sourcePath} />}
        {active.origin.warnings.length > 0 && (
          <MetaRow label="Warnings" value={active.origin.warnings.join("; ")} />
        )}
      </CollapsiblePanel>
        ...
        {active.governance.flags.length > 0 ? (
          <div style={{ marginTop: 8 }}>
            ...
            {active.governance.flags.map((flag, index) => (
              <div
                key={`${flag}-${index}`}
                style={{ display: "flex", alignItems: "flex-start", gap: 6, marginBottom: 5 }}
              >
                <span style={{ color: "#e67e22", fontSize: 11, paddingTop: 1, flexShrink: 0 }}>
                  ⚑
                </span>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 5, marginTop: 2 }}>
            ...
            <span
              style={{ fontSize: 12, color: "#52b788", fontFamily: "'IBM Plex Mono', monospace" }}
            >
              No flags
            </span>
          </div>
...
export function MeaningNavigatorView({
  sections,
  title,
  jurisdiction,
  answerLanguage,
  ...
  return (
    ...
    >
      ...
      {!selected && (
        ...
        >
          {isMobile
            ? "Tap a section to reveal its meaning"
            : "↓ Select a section to reveal its meaning"}
        </div>
      )}
      <div style={{ display: "flex", flex: 1, overflow: "hidden", position: "relative" }}>
        <div
          style={{
            flex: !isMobile && active ? "0 0 52%" : "1 1 100%",
            transition: "flex 0.3s ease",
            overflowY: "auto",
            padding: isMobile ? "24px 16px 120px" : "32px 36px",
            borderRight: !isMobile && active ? "1px solid #d4c9bc" : "none",
          }}
        >
            ...
            {sections.map((section) => {
              ...
              return (
                <div
                  key={section.id}
                  onClick={() =>
                    setSelected((previous) => (previous === section.id ? null : section.id))
                  }
                  ...
                  style={{
                    ...
                    background: isActive ? "#fffaf4" : isHovered ? "#faf7f4" : "transparent",
                    cursor: "pointer",
                ...
                >
                  ...
                  {isActive && (
                    <div
                      style={{
                        height: 2,
                        background: "linear-gradient(90deg, #c17f3a, #e8c99a)",
                        margin: "0 14px 10px",
                      }}
                    />
                  )}
      ...
      {isMobile && active && (
        <>
          ...
          >
            <div
              style={{ padding: "10px 20px 0", flexShrink: 0, borderBottom: "1px solid #e8e0d8" }}
            >
