  ...
  return (
    <div className="space-y-4">
      <Section title="Origin Status">
        <div className="flex flex-wrap gap-2">
          ...
          {hasUnresolvedReferences && (
            <span className={`rounded-full border px-3 py-1.5 text-[10px] font-semibold uppercase tracking-widest ${toneClass("review")}`}>
        ...
        {referencedSources.length > 0 ? (
          <div className="space-y-3">
            {referencedSources.map((source, index) => {
              const referenceStatus = source.status || (hasUnresolvedReferences ? "not_attempted" : "");
              ...
              return (
                <div key={`${source.name}-${index}`} className="rounded-lg border border-border/50 bg-background/40 p-3">
                  ...
                  {referenceStatus && <div className="mt-2 text-xs text-muted-foreground">Retrieval status: {displayStatus(referenceStatus)}</div>}
                  {source.matched_text && <div className="mt-3"><SourceQuote>{source.matched_text}</SourceQuote></div>}
                  {source.why_it_matters && <p className="mt-2 text-sm leading-6 text-muted-foreground">{hideAtomicReferences(source.why_it_matters, atomicReferenceLabel)}</p>}
      ...
      <Section title="Source Signals">
        {signals.length > 0 ? (
          <div className="space-y-3">
            {signals.map((signal, index) => <DetailRow key={`${signal.signal}-${index}`} label={signal.signal} value={hideAtomicReferences(signal.value, atomicReferenceLabel)} />)}
          </div>
